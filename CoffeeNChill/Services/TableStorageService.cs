...
namespace CoffeeNChill.Services
{
    public class TableStorageService : ITableStorageService
    {
        ...
        {
            string connectionString = configuration["AzureWebJobsStorage"] ?? throw new InvalidOperationException("AzureWebJobsStorage connection is missing.");
        ...
        public async Task<bool> DeleteMenuItemAsync(string category, string sku)
        {
            try
            {
                //Retrieve the enity to obtain its flag
                Response<MenuItem> response = await _tableClient.GetEntityAsync<MenuItem>(category, sku);

                //Delete the entity
                await _tableClient.DeleteEntityAsync(category, sku, response.Value.ETag);
                return true;
            }
        ...
        {
            List<MenuItem> menuItems = new List<MenuItem>();
            string filter = $"Partition eq '{category}'";
